...
<template>
    <VPFlyout v-if="!screenMenu" class="VPVersionSwitcher" icon="vpi-versioning" :button="currentVersion" :label="'Switch Version'">
        <div class="items">
            <template v-for="version in sortedVersions" :key="version.value">
                <VPMenuLink v-if="currentVersion != version.value" :item="{
                    text: version.text,
                    link: versionedLink(version.value)
                }" />
            </template>
        </div>
    </VPFlyout>
